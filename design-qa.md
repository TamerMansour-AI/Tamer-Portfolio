...
- The first GitHub Actions run stopped at `npm ci` because the Windows-generated lockfile omitted two cross-platform optional packages. The lockfile now explicitly includes them; a fresh local `npm ci`, build, and 81-page verification passed before retrying the deployment.
